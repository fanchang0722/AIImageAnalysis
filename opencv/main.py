...
# It also stores the maximum pixel intensity value and its location in each image file. 
# It then goes through each image file and rotates a line of specified length around the maximum pixel intensity location 
...
def main():
    ...
    max_intensity_file_index = find_max_intensity_file(file_list, imageFiles, time_diffs)

    populate_imagefiles_model(imageFiles)

    reset_cut_line_for_all_files(imageFiles, max_intensity_file_index)
...
def plot_chart(imageFiles, time_diffs):
    ...
    y = [
        imageFile__.max_avg_brightness - imageFile__.backgroudPointIntensity
    ...
    plt.ylabel('Y-axis: mean(intensity)')
    plt.title('My plot')
...
def draw_line_and_show(imageFiles):
    for imageFile in imageFiles:
        ...
        x1 = int(imageFile.max_intensity_x - line_length / 2 * math.cos(theta))
        y1 = int(imageFile.max_intensity_y - line_length / 2 * math.sin(theta))
        x2 = int(imageFile.max_intensity_x + line_length / 2 * math.cos(theta))
        y2 = int(imageFile.max_intensity_y + line_length / 2 * math.sin(theta))
...
def reset_cut_line_for_all_files(imageFiles, max_intensity_file_index):
    for file_index in range(len(imageFiles)):
        if (max_intensity_file_index != file_index):
            ...
            x = imageFiles[max_intensity_file_index].max_intensity_x
            y = imageFiles[max_intensity_file_index].max_intensity_y

            # Define the angle of rotation in degrees
            angle_radians = math.radians(imageFiles[max_intensity_file_index].angle_of_max_brightness)

            # Calculate the end points of the line
            x1 = int(imageFiles[max_intensity_file_index].max_intensity_x - (line_length / 2) * math.cos(angle_radians))
            y1 = int(imageFiles[max_intensity_file_index].max_intensity_y - (line_length / 2) * math.sin(angle_radians))
            x2 = int(imageFiles[max_intensity_file_index].max_intensity_x + (line_length / 2) * math.cos(angle_radians))
            y2 = int(imageFiles[max_intensity_file_index].max_intensity_y + (line_length / 2) * math.sin(angle_radians))
            ...
            imageFiles[file_index].angle_of_max_brightness = imageFiles[max_intensity_file_index].angle_of_max_brightness
            imageFiles[file_index].max_intensity_x = imageFiles[max_intensity_file_index].max_intensity_x
            imageFiles[file_index].max_intensity_y = imageFiles[max_intensity_file_index].max_intensity_y
...
def populate_imagefiles_model(imageFiles):
    for imageFile in imageFiles:
        ...
        # Define the center point of the line
        x, y = imageFile.max_intensity_x, imageFile.max_intensity_y
        ...
        #imageFile.backgroudPointIntensity = np.mean(region)
        imageFile.backgroudPointIntensity = 0

def find_max_intensity_file(file_list, imageFiles, time_diffs):
    max_intensity = 0
    max_intensity_file_index = 0
    ...
    for i, file_name in enumerate(file_list):
        ...
        imageFile.max_intensity = img.flat[max_value_index]

        if i > 0 and imageFile.max_intensity > max_intensity:
            max_intensity = imageFile.max_intensity
            max_intensity_file_index = i

        # Convert the flattened index back to row and column coordinates
        max_row, max_col = np.unravel_index(max_value_index, img.shape)
        imageFile.max_intensity_x, imageFile.max_intensity_y = max_col, max_row
    ...
    return max_intensity_file_index
...
class ImageFile:
    def __init__(self, fileName, filePath):
        ...
        self.max_intensity_x = None
        self.max_intensity_y = None
